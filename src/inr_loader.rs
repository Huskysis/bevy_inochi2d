...
//!
//! INR's flat pre-order node list, index-based cross-references and raw RGBA8
//! textures let this loader build assets without image decoding, UUID maps or
//! nested-JSON parsing.
...
/// `AssetLoader` for `.inr` - the crate's default/preregistered format.
#[derive(TypePath)]
...
) -> Result<InxPuppet, inr::InrError> {
    let doc = &model.doc;

    // Textures use sRGB views: hardware decodes to linear on sample, blending
    // happens in linear space and the shader premultiplies after sampling. Data must
    // be straight alpha; INR textures flagged premultiplied are un-premultiplied
    // here (in gamma space, matching how they were premultiplied).
...
/// Post-order build (children first), pushing each node into `nodes` so the root
/// ends up last - same contract the spawner expects.
fn build_node(
    model: &InrModel,
    index: usize,
    children: &[Vec<usize>],
    ...
    let mesh = n
        .mesh
        .as_ref()
        .map(|m| convert_mesh(model, m))
        .transpose()?
        .map(|mut mesh| {
            if let Some(contours) = doc.mask_contours.get(&n.uuid) {
                mesh.mask_contour_uv = Some(contours.clone());
            }
            mesh
        });

    // MeshGroups keep their lattice mesh on the node (no material holds it).
    let group_mesh = (node_type == InxNodeType::MeshGroup)
        .then(|| mesh.clone().map(Arc::new))
        .flatten();
    ...
    let material = if let Some(part) = &n.part {
        ...
        n.composite.as_ref().map(|comp| InxMaterial {
            mesh: None,
            texture_albedo: None,
    ...
    let asset = InxNode {
        ...
        node_type,
        mesh: group_mesh,
        mesh_group_dynamic: n.mesh_group_dynamic,
        composite_propagate_meshgroup: n
            .composite
            .as_ref()
            .map(|c| c.propagate_meshgroup)
            .unwrap_or(true),
        material,
        transform: Transform {
        ...
        enabled: n.enabled,
        physics_data,
        compose_hint: n.composite.as_ref().and_then(|c| {
            c.compose_hint.map(|h| match h {
                inr::InrComposeHint::ChildrenDisjoint => {
                    composite::InxComposeHint::ChildrenDisjoint
                }
                inr::InrComposeHint::ChildrenOverlap => {
                    composite::InxComposeHint::ChildrenOverlap
                }
            })
        }),
